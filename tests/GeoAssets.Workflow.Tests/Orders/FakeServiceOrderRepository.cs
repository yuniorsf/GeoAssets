...
/// <summary>
/// Full-featured <see cref="IServiceOrderRepository"/> test double — a real dictionary-backed
/// store with working filters and event-raising, used wherever a test needs a genuine inner
/// repository to delegate/forward through (not a stub). Replaces the old
/// <c>InMemoryServiceOrderRepository</c>, removed from production in XD01-129.
///
/// Deliberately does <b>not</b> validate transitions on <see cref="UpdateAsync"/>/
/// <see cref="AppendActionAsync"/> — callers that need that covered use
