...
/// <c>/api/identity/*</c> endpoints (XD01-18) — <c>GeoAssets.Web</c>'s sole identity
/// registration (XD01-130 removed the earlier in-memory dev/demo alternative).
///
...
/// config; <see cref="IRoleSyncStatusProvider"/> is how the UI tells which).
///
/// Also registers <see cref="IPendingInvitationRepository"/>/<see cref="IUserClaimRepository"/>
/// (XD01-70) and <see cref="IInvitationStatusProvider"/>/<see cref="IInvitationClient"/>
...
/// endpoints. <see cref="IPolicyRepository"/> is also registered here — like
/// <see cref="IPermissionRepository"/>, policies are code-seeded and read-only
